...
def load_cuda_kernels(t_max = 1024, b_group_forward = 8, b_group_backward = 2):
    ...
    try:
        ...
        timex_cuda = load(
            name="timex", src_files, verbose=True, extra_cuda_cflags=[
                        '--use_fast_math', 
...
def load_tf_weights_in_rwkv2(model, config, tf_checkpoint_path):
    ...
    raise NotImplementedError('Not supported for RWKV models')
    return
...
class RWKV2Model(RWKV2PreTrainedModel):
    ...
    def forward(self, idx, targets=None):
        ...
        hidden_states = self.h(x)

        output_layer = self.ln_out(x)
