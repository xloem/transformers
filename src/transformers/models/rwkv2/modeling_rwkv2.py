...
_TOKENIZER_FOR_DOC = "RWKV2Tokenizer"
...
def load_tf_weights_in_rwkv2(model, config, tf_checkpoint_path):
    ...
    raise NotImplementedError('Not yet supported for RWKV models')
    return
...
class RWKV2Embeddings(nn.Module):
...
class RWKV2SelfAttention(nn.Module):

    def __init__(self, config, position_embedding_type=None):
        raise NotImplementedError('RWKV models are attention-free')
        return
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in RWKV2Model forward() function)
...
class RWKV2SelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class RWKV2Attention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        raise NotImplementedError('RWKV models are attention-free')
        return

        super().__init__()
        self.self = RWKV2SelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = RWKV2SelfOutput(config)
...
class RWKV2Intermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class RWKV2Layer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RWKV2Attention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            assert self.is_decoder, f"{self} should be used as a decoder model if cross attention is added"
            self.crossattention = RWKV2Attention(config, position_embedding_type="absolute")
        self.intermediate = RWKV2Intermediate(config)
        self.output = RWKV2Output(config)
...
class RWKV2Encoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RWKV2Layer(config) for _ in range(config.num_hidden_layers)])
...
class RWKV2PredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class RWKV2LMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = RWKV2PredictionHeadTransform(config)
...
class RWKV2OnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = RWKV2LMPredictionHead(config)
...
@add_start_docstrings(
    "The bare RWKV2 Model transformer outputting raw hidden-states without any specific head on top.",
...
class RWKV2Model(RWKV2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = RWKV2Embeddings(config)
        self.encoder = RWKV2Encoder(config)
        ...
        self.blocks = nn.Sequential(*[RWKV2_Block(i) for i in range(n_layer)])
...
@add_start_docstrings("""RWKV2 Model with a `language modeling` head on top. """, RWKV2_START_DOCSTRING)
class RWKV2ForMaskedLM(RWKV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        if config.is_decoder:
            logger.warning(
                "If you want to use `RWKV2ForMaskedLM` make sure `config.is_decoder=False` for "
...
class RWKV2ClassificationHead(nn.Module):
...
@add_start_docstrings(
    """RWKV2 Model transformer with a sequence classification/regression head on top (a linear layer on top of
    the pooled output) e.g. for GLUE tasks. """,
    RWKV2_START_DOCSTRING,
)
class RWKV2ForSequenceClassification(RWKV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.rwkv2 = RWKV2Model(config)
        self.classifier = RWKV2ClassificationHead(config)
...
@add_start_docstrings(
    """RWKV2 Model with a multiple choice classification head on top (a linear layer on top of
    the pooled output and a softmax) e.g. for RocStories/SWAG tasks. """,
    RWKV2_START_DOCSTRING,
)
class RWKV2ForMultipleChoice(RWKV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_start_docstrings_to_model_forward(RWKV2_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """RWKV2 Model with a token classification head on top (a linear layer on top of
    the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks. """,
    RWKV2_START_DOCSTRING,
)
class RWKV2ForTokenClassification(RWKV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
@add_start_docstrings(
    """RWKV2 Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layers on top of the hidden-states output to compute `span start logits` and `span end logits`). """,
    RWKV2_START_DOCSTRING,
)
class RWKV2ForQuestionAnswering(RWKV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
